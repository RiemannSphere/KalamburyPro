...
public class ChatWebsocket2 {
	...
	private static Set<ChatWebsocket2> endpoints = new CopyOnWriteArraySet<>();
	...
	public void onMessage(Session s, String message) {
		...
		System.out.println("ALL SESSIONS: " + s.getOpenSessions().size());
		...
		if (msg.getMsgType().equals(MsgType.CLEAN_CANVAS.getValue())) {
			...
			for (ChatWebsocket2 user : endpoints) {
				try {
	...
	/**
	 * Execute in case the drawing user is changed or new word is generated
	 * 
	...
	private void setNextUserForDrawing(Session nextDrawingUser) {
	...
	private void cleanWordToGuessForAll() throws IOException {
		chatService.cleanCurrentWordToGuess();
	...
	private void sendNextWordToGuess() throws IOException {
		...
		System.out.println("Chat Websocket: current session id: " + this.session.getId());
		System.out.println("Chat Websocket: drawing user session id: " + chatService.getCurrentUserDrawing().getId());
		System.out.println("Chat Websocket: is current session open: " + this.session.isOpen());
		System.out.println("Chat Websocket: is drawing user session open: " + chatService.getCurrentUserDrawing().isOpen());
	...
	private void processMessage(String msg, Session msgSender)
